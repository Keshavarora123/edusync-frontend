...
const CourseAssessments = () => {
  ...
  const handleCreateAssessment = async (assessmentData) => {
    try {
      ...
      console.log('Raw assessment data before processing:', JSON.stringify(assessmentData, null, 2));
      
      // Transform the questions to ensure correctOption is properly set
      const transformedQuestions = assessmentData.questions.map(question => {
        console.log('Processing question:', question);
        return {
          ...question,
          // Ensure correctOption is a string and uppercase (A, B, C, or D)
          correctOption: question.correctOption ? question.correctOption.toString().toUpperCase() : null
        };
      });
      
      // Create the final assessment data with transformed questions
      ...
      console.log('Sending assessment data to API:', JSON.stringify(assessmentWithCourse, null, 2));
      
      // Send to the API
      const response = await createAssessment(assessmentWithCourse);
